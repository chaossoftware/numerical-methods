...
/// </summary>
public sealed class LleRosenstein : ITimeSeriesLyapunov, IHasDescription
{
    ...
    public override string ToString() =>
        ...
        .AppendLine($" - min ε = {NumFormat.Format(epsilon)}")
        .ToString();
    ...
    public string Description =>
        new StringBuilder()
        .AppendLine($"Largest Lyapunov exponent by Rosenstein [{Paper}]")
    ...
    public void Calculate(double[] timeSeries)
    {
        ...
        for (eps = epsilon; !alldone; eps *= 1.1)
        {
            ...
            Log.Debug("{0:F5}\t\t{1}", eps * interval, _found[0]);
        }
