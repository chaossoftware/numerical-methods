...
/// </summary>
public sealed class LeSpecSanoSawada : ITimeSeriesLyapunov, IHasDescription
{
    ...
    public void Calculate(double[] timeSeries)
    {
        ...
        for (i = (_eDim - 1) * _tau; i < start; i++)
        {
            ...
            if (timer.ElapsedMilliseconds > OutputInterval || (i == (start - 1)))
            {
                ...
                string logEntry = $"{count} ";

                for (j = 0; j < _eDim; j++)
                {
                    logEntry += $"{NumFormat.Format(factor[j] / count)} ";
        ...
        Log.Info("");
        Log.Info("Avg. rel. forecast error = {0}", NumFormat.Format(Math.Sqrt(errorAvg / count) / variance));
        Log.Info("Avg. abs. forecast error = {0}", NumFormat.Format(Math.Sqrt(errorAvg / count) * interval));
        Log.Info("Avg. neighborhood size   = {0}", NumFormat.Format(epsAvg * interval / count));
        Log.Info("Avg. number of neighbors = {0}", NumFormat.Format(neighborsAvg / count));
    }
    ...
    private void MakeDynamics(double[] series, BoxAssistedFnn fnn, double[] dynamics, int act)
    {
        ...
        Vector.FillWith(vector, 0d);
        Matrix.FillWith(matrix, 0d);
