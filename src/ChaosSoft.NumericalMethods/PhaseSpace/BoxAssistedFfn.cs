...
namespace ChaosSoft.NumericalMethods.PhaseSpace
{
    internal class BoxAssistedFnn
    {
        ...
        internal int FindNeighborsK(double[] series, int eDim, int tau, double epsilon, int act, int window)
        {
            ...
            int nf = 0;
            int x, y, i, i1, j, k, k1;
            double dx, eps2 = Numbers.QuickPow2(epsilon);
            ...
            for (i = x - 1; i <= x + 1; i++)
            {
                ...
                for (j = y - 1; j <= y + 1; j++)
                {
                    ...
                    while (element != -1)
                    {
                        ...
                        {
                            dx = Numbers.QuickPow2(series[act] - series[element]);

                            if (dx <= eps2)
                            {
                                k = eDim - 1;
                                k1 = k * tau;
                                dx += Numbers.QuickPow2(series[act + k1] - series[element + k1]);
        ...
        internal bool FindNeighborsR(double[] timeSeries, int eDim, int tau, double epsilon, long act, int window, out int minelement)
        {
            ...
            double dx, eps2 = Numbers.QuickPow2(epsilon), mindx = 1.0;
                ...
                for (int j = y - 1; j <= y + 1; j++)
                {
                    ...
                    while (element != -1)
                    {
                        if (Math.Abs(act - element) > window)
                        {
                            ...
                            {
                                dx += Numbers.QuickPow2(timeSeries[act + k] - timeSeries[element + k]);
