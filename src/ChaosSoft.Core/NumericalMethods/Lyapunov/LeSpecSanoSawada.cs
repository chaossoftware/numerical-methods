...
namespace ChaosSoft.Core.NumericalMethods.Lyapunov
{
    ...
    /// </summary>
    public class LeSpecSanoSawada : LyapunovMethod
    {
        ...
        public LeSpecSanoSawada(double[] timeSeries, int eDim, int tau, int iterations, double scaleMin, double epsstep, int minNeigh, bool inverse)
            : base(timeSeries)
        {
            _eDim = eDim;
            _tau = tau;
        ...
        public LeSpecSanoSawada(double[] timeSeries) : this(timeSeries, 2, 1, timeSeries.Length, 0, 1.2, 30, false) // 3rd from end parameter (eps) is 0 to obtain further it's default value
        {
        }

        public LeSpecSanoSawada(double[] timeSeries, int eDim) : this(timeSeries, eDim, 1, timeSeries.Length, 0, 1.2, 30, false) // 3rd from end parameter (eps) is 0 to obtain further it's default value
        {
        }
