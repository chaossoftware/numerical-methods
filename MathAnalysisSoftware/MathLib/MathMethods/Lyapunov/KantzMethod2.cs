...
namespace MathLib.MathMethods.Lyapunov {
    public class KantzMethod2 : LyapunovMethod {
        ...
        public KantzMethod2(double[] timeSeries, int dimMin, int dimMax, int tau, int maxIterations, int window, double scaleMin, double scaleMax, int epscount)
            :base(timeSeries) {
        ...
        public override void Calculate() {
            ...
            DimMax = Math.Max(DimMax, 2);
            DimMin = Math.Max(DimMin, 2);
            DimMax = Math.Max(DimMax, DimMin);
  
            liste = new long[timeSeries.Length];
            found = new long[DimMax - 1];
            lfound = new long[DimMax - 1, timeSeries.Length];
            count = new long[DimMax - 1, MaxIterations + 1];
            lyap = new double[DimMax - 1, MaxIterations + 1];

            if (Epscount == 1)
                eps_fak = 1d;
            else
                eps_fak = Math.Pow(Epsmax / Epsmin, 1d / (double)(Epscount - 1));
        ...
        private void PutInBoxes(double eps) {
            ...
            for (i = 0; i < blength; i++) {
                j = (long)(timeSeries[i] / eps) & ibox;
                k = (long)(timeSeries[i + Tau] / eps) & ibox;
                liste[i] = box[j, k];
                box[j, k] = i;
            }
        }
        ...
        private void LfindNeighbors(long act, double eps) {
            ...
            //for (hi = 0; hi < DimMax - 1; hi++)
            //    found[hi] = 0;

            i = (long)(timeSeries[act] / eps) & ibox;
            j = (long)(timeSeries[act + Tau] / eps) & ibox;
